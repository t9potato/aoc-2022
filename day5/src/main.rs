...
fn part1(input: &str) -> String {
    ...
    let _ = input.next().unwrap().lines().for_each(|line|{
        for symbol in line.chars().enumerate().filter(|char| char.1.is_alphabetic()).collect::<Vec<(usize, char)>>() {
            let mut input = crane.get(&((symbol.0 - 1)/ 4)).unwrap_or(&Vec::new()).clone();
            input.push(symbol.1);
